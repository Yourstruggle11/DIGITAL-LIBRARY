...
import "../style/imageDocumentPageStyle.css";
import SearchIcon from '@material-ui/icons/Search';
import ImageDocumentCard from "../components/ImageDocumentCard"
import Preloader from "../components/Preloader"
import { useDispatch, useSelector } from "react-redux";
import {imageShowToUser} from "../redux/actions/imageAction"

export default function ImageDocumentScreen() {
    const dispatch = useDispatch();
    const { books,loading } = useSelector((state) => state.imageUpload);
//   console.log(books);
    useEffect(() => {
      dispatch(imageShowToUser())
    }, [dispatch])
   

    function mapFun(curele){
        // console.log( "date",curele.Date);
        return(
            <ImageDocumentCard key={curele._id} name={curele.fileName} uploaderName={curele.adminName} uploadDate={curele.Date} link={curele.LINK} />
        )
    }
